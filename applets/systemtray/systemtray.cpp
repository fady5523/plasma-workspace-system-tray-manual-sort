...
void SystemTray::ensureManualOrderContainsAllItems()
{
    ...
    for (int i = 0; i < model->rowCount(); ++i) {
        ...
        QString id = idx.data(static_cast<int>(BaseModel::BaseRole::ItemId)).toString();
    ...
    if (changed) {
        m_settings->setManualOrder(order);
    }
}

SortedSystemTrayModel::SortingType SystemTray::sortingTypeForCurrentMode() const
{
    if (!m_settings) {
        return SortedSystemTrayModel::SortingType::SystemTray;
    }
    if (m_settings->orderingMode() == QLatin1String("custom")) {
        return SortedSystemTrayModel::SortingType::ManualOrder;
    }
    return SortedSystemTrayModel::SortingType::SystemTray;
}

SortedSystemTrayModel::SortingType SystemTray::configSortingTypeForCurrentMode() const
{
    if (!m_settings) {
        return SortedSystemTrayModel::SortingType::ConfigurationPage;
    }
    if (m_settings->orderingMode() == QLatin1String("custom")) {
        return SortedSystemTrayModel::SortingType::ManualOrder;
    }
...
QAbstractItemModel *SystemTray::sortedSystemTrayModel()
{
    if (!m_sortedSystemTrayModel) {
        m_sortedSystemTrayModel = new SortedSystemTrayModel(sortingTypeForCurrentMode(), m_settings, this);
        m_sortedSystemTrayModel->setSourceModel(systemTrayModel());
        if (sortingTypeForCurrentMode() == SortedSystemTrayModel::SortingType::ManualOrder) {
            ensureManualOrderContainsAllItems();
        }
        connect(m_settings, &SystemTraySettings::configurationChanged, this, [this]() {
            auto type = sortingTypeForCurrentMode();
            if (type == SortedSystemTrayModel::SortingType::ManualOrder) {
                ensureManualOrderContainsAllItems();
            }
            m_sortedSystemTrayModel->setSortingType(type);
        });
    }
...
QStringList SystemTray::manualOrder() const
{
    return m_settings ? m_settings->manualOrder() : QStringList();
}

QAbstractItemModel *SystemTray::configSystemTrayModel()
{
    if (!m_configSystemTrayModel) {
        m_configSystemTrayModel = new SortedSystemTrayModel(configSortingTypeForCurrentMode(), m_settings, this);
        m_configSystemTrayModel->setSourceModel(systemTrayModel());
        if (configSortingTypeForCurrentMode() == SortedSystemTrayModel::SortingType::ManualOrder) {
            ensureManualOrderContainsAllItems();
        }
        connect(m_settings, &SystemTraySettings::configurationChanged, this, [this]() {
            auto type = configSortingTypeForCurrentMode();
            if (type == SortedSystemTrayModel::SortingType::ManualOrder) {
                ensureManualOrderContainsAllItems();
            }
            m_configSystemTrayModel->setSortingType(type);
        });
    }
